...
class_name QuadTreeMeshMinecraft
...
var equator_plane_normal = Vector3(0, 1, 0)
...
var noiseValues = PackedVector4Array()
...
func update_vertex_and_save_data(vertex):
	var continentalness = perlin_noise_generators["continentalness"].getValue(vertex)
	var erosion = perlin_noise_generators["erosion"].getValue(vertex)
	var peaks_valleys = perlin_noise_generators["peaks_valleys"].getValue(vertex)
	var temperature_random = perlin_noise_generators["temperature"].getValue(vertex)
	var temperature_sun_dependent = 1 - abs(vertex.normalized().dot(equator_plane_normal))
	noiseValues.append(Vector4(continentalness, erosion, peaks_valleys, temperature_random))
	vertex += ((continentalness + peaks_valleys) * (1.0 - erosion) * max_height_difference) * vertex.normalized()
	heightValues.append(Vector4(vertex.length(), temperature_sun_dependent, 0, 0))
	if vertex.length() < ocean_level:
		vertex = vertex.normalized() * ocean_level
	return vertex
...
func create_mesh_and_collision(mesh_generation_data, root_node, node_id):
	...
	self.perlin_noise_generators = mesh_generation_data["perlin_noise_generators"]
	self.ocean_level = mesh_generation_data["ocean_level"]
	self.base_height = mesh_generation_data["base_height"]
	self.max_height_difference = mesh_generation_data["max_height_difference"]
	self.material = mesh_generation_data["material"]
	self.equator_plane_normal = mesh_generation_data["equator_plane_normal"]
	...
	var point0 = (self.base_normal + (percent_negative.x - 0.5) * 2.0 * self.axis_a + (percent_negative.y - 0.5) * 2.0 * self.axis_b).normalized() * base_height
	var point1 = (self.base_normal + (percent_negative.x - 0.5) * 2.0 * self.axis_a + (percent_positive.y - 0.5) * 2.0 * self.axis_b).normalized() * base_height
	var point2 = (self.base_normal + (percent_positive.x - 0.5) * 2.0 * self.axis_a + (percent_negative.y - 0.5) * 2.0 * self.axis_b).normalized() * base_height
	var point3 = (self.base_normal + (percent_positive.x - 0.5) * 2.0 * self.axis_a + (percent_positive.y - 0.5) * 2.0 * self.axis_b).normalized() * base_height
	...
	quad_base_points = [
		point0 + ((perlin_noise_generators["continentalness"].getValue(point0) + 
				   perlin_noise_generators["peaks_valleys"].getValue(point0)) * (1.0 - perlin_noise_generators["erosion"].getValue(point0)) * max_height_difference) * point0.normalized(),
		point1 + ((perlin_noise_generators["continentalness"].getValue(point1) + 
				   perlin_noise_generators["peaks_valleys"].getValue(point1)) * (1.0 - perlin_noise_generators["erosion"].getValue(point1)) * max_height_difference) * point1.normalized(),
		point2 + ((perlin_noise_generators["continentalness"].getValue(point2) + 
				   perlin_noise_generators["peaks_valleys"].getValue(point2)) * (1.0 - perlin_noise_generators["erosion"].getValue(point2)) * max_height_difference) * point2.normalized(),
		point3 + ((perlin_noise_generators["continentalness"].getValue(point3) + 
				   perlin_noise_generators["peaks_valleys"].getValue(point3)) * (1.0 - perlin_noise_generators["erosion"].getValue(point3)) * max_height_difference) * point3.normalized()
	]
	...
	for i in range(vertices.size()):
		...
		surface_tool.set_custom(0, Color(noiseValues[i].x, noiseValues[i].y, noiseValues[i].z, noiseValues[i].w))
		surface_tool.set_custom(1, Color(heightValues[i].x, heightValues[i].y, heightValues[i].z, heightValues[i].w))
	...
	root_node.add_child.call_deferred(self.mesh_instance)
...
func get_mesh_generation_data_for_next(x, y, resolution, u_pos, v_pos):
	...
	return {
		"perlin_noise_generators": self.perlin_noise_generators,
		"x": x,
		"y": y,
		...
		"equator_plane_normal": self.equator_plane_normal
	}
...
func spawn_tree(tree_position, tree_type):
	...
	tree_instance.call_deferred("add_to_group", "Trees")
...
func get_fitting_tree_type(tree_position):
	# use original height 
	var continentalness = perlin_noise_generators["continentalness"].getValue(tree_position.normalized() * base_height)
	if (continentalness < 0.37):
		return "palm"
	else:
		return "normal"
