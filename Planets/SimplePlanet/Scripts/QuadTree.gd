extends Node
class_name QuadTreeSimple

var mesh : QuadTreeMeshSimple
var node_id = -1
var root_node = null
...
func split_if_too_close(camera_pos):
	if b_is_split == false:
		if camera_pos.distance_to(mesh.center_side) < THRESHOLDS[current_split] * mesh.base_radius:
			var new_resolution = (mesh.current_resolution - 1) * 2 + 1
			quads.append(QuadTreeSimple.new(current_split+1, 
...
func spawn_trees():
	...
	if mesh.center_side.length() < config.get_value("planet", "min_normal_tree_height"):
		num_trees = randi() % (MAX_NUM_TREES + 1 - MIN_NUM_TREES) + MIN_NUM_TREES
		tree_type = "palm"
	elif mesh.center_side.length() < config.get_value("planet", "min_forest_normal_tree_height"):
		num_trees = randi() % (MAX_NUM_TREES + 1 - MIN_NUM_TREES) + MIN_NUM_TREES
		tree_type = "normal"
	else:
		num_trees = (randi() % (MAX_NUM_TREES + 1 - MIN_NUM_TREES) + MIN_NUM_TREES) * 3
		tree_type = "normal"
	mesh.get_random_position_on_mesh()
	for i in range(num_trees):
		var tree_position = Vector3(mesh.get_random_position_on_mesh())
		# make sure that they dont spawn in water
		if tree_position.length() < config.get_value("planet", "water_level_relative") * config.get_value("planet", "radius"):
			continue
